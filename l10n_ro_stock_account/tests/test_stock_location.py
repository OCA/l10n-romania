...
class TestStockLocation(TestStockCommon):
    ...
    def test_internal_transit_with_transfer_account(self):
        """Test transfer intre doua depozite prin tranzit cu cont de transfer configurat.

        Scenariul:
        - Se configureaza l10n_ro_property_stock_transfer_account_id pe companie
        - Se face un transfer: depozit_sursa -> tranzit (internal_transit_in SVL = iesire din gestiune)
        - Se face un transfer: tranzit -> depozit_destinatie (internal_transit_out SVL = intrare in gestiune)
        - Se verifica ca notele contabile sunt corecte:
          internal_transit_in (depozit->tranzit): gestiunea_sursa (credit) -> cont_transfer (debit)
          internal_transit_out (tranzit->depozit): cont_transfer (credit) -> gestiunea_destinatie (debit)
        """
        company = self.env.company
        # Asiguram ca avem cont de transfer configurat
        self.assertTrue(
            self.account_stock_transfer,
            "Contul de transfer (l10n_ro_property_stock_transfer_account_id) nu este configurat",
        )
        ...
        # Transfer din depozit sursa -> tranzit (genereaza SVL internal_transit_out)
        self.transfer(self.location_warehouse, self.location_transit, product=self.product_mp)
        picking_out = self.picking

        # Verificam ca s-a generat SVL de tip internal_transit_in (iesire din gestiune spre tranzit)
        ...
        self.assertTrue(svl_out, "Nu s-a generat SVL de tip internal_transit_in (depozit->tranzit)")
        ...
        for svl in svl_out:
            ...
            if am_lines:
                debit_accounts = am_lines.filtered(lambda l: l.debit > 0).mapped("account_id")
                credit_accounts = am_lines.filtered(lambda l: l.credit > 0).mapped("account_id")
                self.assertIn(
                    self.account_stock_transfer,
                    debit_accounts,
                ...
                self.assertIn(
                    ...
                    credit_accounts,
                    "Contul de gestiune sursa trebuie sa fie pe credit in nota transit_out",
                )

        # Transfer din tranzit -> depozit destinatie (genereaza SVL internal_transit_in)
        self.transfer(self.location_transit, self.location_warehouse_other, product=self.product_mp)
        picking_in = self.picking

        # Verificam ca s-a generat SVL de tip internal_transit_out (intrare in gestiune din tranzit)
        ...
        self.assertTrue(svl_in, "Nu s-a generat SVL de tip internal_transit_out (tranzit->depozit)")
        ...
        for svl in svl_in:
            ...
            if am_lines:
                ...
                self.assertIn(
                    self.account_valuation,
                    debit_accounts,
                    "Contul de gestiune destinatie trebuie sa fie pe debit in nota transit_in",
                )
                self.assertIn(
                    self.account_stock_transfer,
                    credit_accounts,
