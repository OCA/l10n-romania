...
class StockValuationLayer(models.Model):
    ...
    l10n_ro_track_return_ids = fields.One2many("l10n.ro.stock.valuation.layer.tracking", "svl_return_id")
    #compute set retururi pe svl orig.
    l10n_ro_svl_return_ids = fields.Many2many("stock.valuation.layer", compute="_compute_l10n_ro_svl_tracking_return")
    l10n_ro_qty_returned = fields.Float(compute="_set_returned_qty", store=True)
    ...
    def _check_return_permision(self):
        ...
        if self.l10n_ro_location_dest_id.usage in ['consume']:
        ...
        elif abs(qty_needed) in [abs(target_qty),
                                abs(available),
                                self.remaining_qty,
                                (available + self.remaining_qty),
                                (target_qty + self.remaining_qty)]:
            ...
            return (False, "First you must to reverse %s %s" % (
                self.stock_move_id.display_name, 
                self.getDocMove(move=self.stock_move_id)
                ), self)
        return (None, None, self)
        
    def _colector_return_available(self, down_stream=[]):
...
class ProductProduct(models.Model):
    ...
    def _prepare_out_svl_vals(self, quantity, company):
        ...
        if self._context.get('origin_return_candidates', None):
            return self._l10n_ro_prepare_out_svls_vals_return(quantity, company, self._context.get('origin_return_candidates'))
        return super(ProductProduct, self)._prepare_out_svl_vals(quantity, company)
    ...
    def _convert_origin_svls(self, quantity, company, svls):
        ...
        for s in svls:
            ...
            move_svls = s.l10n_ro_svl_track_dest_ids.mapped("svl_dest_id.l10n_ro_svl_return_ids").filtered(lambda x: not x.l10n_ro_svl_track_dest_ids)
