...
class AccountEdiDocument(models.Model):
    ...
    l10n_ro_response_state = fields.Selection([('success', 'Success'), ('error', 'Error')], "Response State", copy=False, tracking=True)
    l10n_ro_message_state = fields.Selection([('found_ok', 'Found - Ok'), ('missing', 'Missing'), ('found_notok', 'Found - Not Ok'), ('found_processing', 'Found - Pending')], "Message State", copy=False)
    l10n_ro_response_attachment_id = fields.Many2one('ir.attachment', "Response ZIP", copy=False, help="The file received. Contains either the original invoice or a list of error found for the invoice during processing.")
    l10n_ro_format_code = fields.Char('Format Code', related='edi_format_id.code', store=True, copy=False)

    def _cron_get_response_web_service(self):
        """ Get all documents with l10n_ro_edi_transaction and without response_attachment_id """

        documents = self.search([('l10n_ro_edi_transaction', '!=', False), ('l10n_ro_response_attachment_id', '=', False), ('move_id', '!=', False), ('edi_format_id', '!=', False)])
    ...
    def _process_step_2_response(self, response):
        ...
        if response.get('error'):
            vals.update({'l10n_ro_response_state': 'error', 'error': "{0}<br/>{1}".format(self.error or '', response['error'])})
