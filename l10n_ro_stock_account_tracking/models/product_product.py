    ...
    def _l10n_ro_run_fifo_vacuum(self, company=None):
        ...
        for location in svls_to_vacuum.mapped("l10n_ro_location_id"):
            svl_loc_to_vaccum = svls_to_vacuum.filtered(
                lambda r, location=location: r.l10n_ro_location_id.id == location.id
            )
            loc_candidates = all_candidates.filtered(
                lambda r, location=location: r.l10n_ro_location_dest_id.id
                == location.id
            )
            if not loc_candidates:
                ...
                loc_candidates = all_candidates.filtered(
                    lambda r,
                    parent_locations=parent_locations: r.l10n_ro_location_dest_id.id
            ...
            for svl_to_vacuum in svl_loc_to_vaccum:
                ...
                candidates = loc_candidates.filtered(
                    lambda r, svl_to_vacuum=svl_to_vacuum: r.id > svl_to_vacuum.id
                )
                if not candidates:
                    break
