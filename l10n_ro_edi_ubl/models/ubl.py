...
class BaseUbl(models.AbstractModel):
    ...
    def _ubl_add_party_tax_scheme(
        ...
        if commercial_partner.vat:
            ...
            if not commercial_partner.country_id:
                raise UserError(
                    _("Partner %s without country") % commercial_partner.name
                )
            company_id.text = (
                commercial_partner.country_id.code
                + commercial_partner.vat.replace(commercial_partner.country_id.code, "")
            )
    ...
    def _ubl_add_delivery_terms(self, incoterm, parent_node, ns, version="2.1"):
        ...
        delivery_term_id = etree.SubElement(
            delivery_term, ns["cbc"] + "ID", schemeID="INCOTERM"
        )
