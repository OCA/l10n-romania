...
class TestStreetFields(TransactionCase):
    ...
    def create_and_assert(self, partner_name, country_id, street, street_name,
                          street_number, street_staircase, street_number2):
        ...
        self.assertEqual(
            ...
            street_name,
            'wrong street name for %s: %s' % (
                partner_name, partner.street_name))
        self.assertEqual(
            partner.street_number or '',
            street_number,
            'wrong house number for %s: %s' % (
                partner_name, partner.street_number))
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            street_number2,
            'wrong door number for %s: %s' % (
                partner_name, partner.street_number2))
        partner = self.Partner.create({
            'name': partner_name + '-2',
        ...
        self.assertEqual(
            partner.street or '',
            street,
            'wrong street for %s: %s' % (partner_name, partner.street))
        return partner

    def write_and_assert(self, partner, vals, street, street_name,
                         street_number, street_staircase, street_number2):
        partner.write(vals)
        self.assertEqual(
            partner.street_name or '',
        ...
        self.assertEqual(
            partner.street_staircase or '',
            street_staircase,
            'wrong house number: %s' % partner.street_staircase)
        self.assertEqual(
            partner.street_number2 or '',
            street_number2,
