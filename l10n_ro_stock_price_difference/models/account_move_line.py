...
class AccountMoveLine(models.Model):
    ...
    def l10n_ro_modify_stock_valuation(self, val_dif):
        ...
        # for traceability only: does not influence the valuation, which is
        # already scoped to valuation_stock_move via the context above
        lc.picking_ids = [(6, 0, valuation_stock_move.picking_id.ids)]
        ...
        # link back all the layers (and their journal entries) generated by
        # this landed cost to the source invoice, so the accounting entry
        # keeps a trace to the document that triggered the price difference
        (lc.stock_valuation_layer_ids - svl).write(
            {
                "l10n_ro_invoice_line_id": self.id,
                "l10n_ro_invoice_id": self.move_id.id,
            }
    ...
    def l10n_ro_prepare_price_difference_landed_cost(self, value):
        ...
        return dict(
            ...
            vendor_bill_id=(self.move_id.move_type == "in_invoice" and self.move_id.id),
            cost_lines=[
                (
                    0,
                    0,
