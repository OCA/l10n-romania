...
class MessageSPV(models.Model):
    ...
    def get_xml_fom_zip(self):
        ...
        for message in self:
            ...
            values = message.process_xml(xml_tree)
            ...
            if message.invoice_id:
                ...
                if not message.invoice_id.l10n_ro_edi_document_ids:
                    if message.message_type == "error":
                        ...
                        #   did not upload it (the document's key_loading is not
                        #   what the fetch reads). The fetch fails on every run,
                        #   logs in the chatter, and re-triggers itself every 2
    ...
    def create_invoice(self):
        ...
        for message in self.filtered(lambda m: not m.invoice_id):
            ...
            try:
                new_invoice._extend_with_attachments(attachment_xml)
            except Exception as e:
    ...
    def _render_anaf_pdf_bytes(self, no_validate=None):
        ...
        if not xml:
            raise UserError(_("No XML found in the downloaded ZIP."))
    ...
    def action_download_embedded_pdf(self):
        ...
        if not pdf_bytes:
            raise UserError(_("This invoice's XML does not contain an embedded PDF."))
