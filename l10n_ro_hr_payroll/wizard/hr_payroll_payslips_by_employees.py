...
class HrPayslipEmployees(models.TransientModel):
    ...
    def compute_sheet(self):
        ...
        for employee in self.env['hr.employee'].browse(data['employee_ids']):
            ...
            contracts = self.env['hr.payslip'].get_contract(employee, from_date, to_date)
            for contract in self.env['hr.contract'].browse(contracts):
                ...
                slip_data = self.env['hr.payslip'].onchange_employee_id(contract_from_date, contract_to_date, employee.id, contract_id=False)
                res = {
