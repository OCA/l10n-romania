...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="stylesheet">
				...
				<paraStyle name="name" fontName="Helvetica" textColor="green" fontSize="9"/>
				<paraStyle name="other" fontName="Helvetica" fontSize="8" alignment="CENTER"/>
				<paraStyle name="normal" fontName="Helvetica" fontSize="6.5" alignment="CENTER"/>
    ...
    <xsl:template name="story">
        ...
        <blockTable>
            ...
            <xsl:for-each select="report/user">
				<xsl:variable name="user1" select="position()"/>
				<tr>
					<td>
						<para style="name"><xsl:value-of select="name"/></para>
						<para style="other"><xsl:value-of select="program"/></para>
					</td>
					<xsl:for-each select="../days/dayy">
						<xsl:variable name="num" select="attribute::number"/>
						<td>
							<xsl:for-each select="../../user[$user1]/day">
								<xsl:if test="number(num) = number($num)">
									<para style="normal"><xsl:value-of select="hour_from"/></para>
									<para style="normal"><xsl:value-of select="hour_to"/></para>
								</xsl:if>
							</xsl:for-each>
						</td>
					</xsl:for-each>
    ...
    <xsl:template match="title">
        ...
        <para style="title"><xsl:value-of select="/report/user[1]/departament" /></para>
        <para style="title"><xsl:value-of select="/report/month" /> - <xsl:value-of select="/report/year" /></para>
    </xsl:template>
</xsl:stylesheet>
