...
class report_custom(report_rml):
    ...
    def create_xml(self, cr, uid, ids, datas, context=None):
        ...
        dy = lengthmonth(month.year, month.month)
        if emp_ids:
            for emp in obj_emp.read(cr, uid, emp_ids, ['name']):
                ...
                while today.month == month.month:
                    ...
                    for att in attendences:
                        ...
                        leave = ''
                        if att['action'] not in ('sign_out', 'sign_in'):
                            leave = att['action']
                        if att['action']=='sign_in':
                            hour_from = datetime.strptime(att['name'],'%Y-%m-%d %H:%M:%S')
                        if ldt and att['action'] == 'sign_out':
                            wh += (float((dt - ldt).seconds)/60/60)
                            if not hour_from:
                                hour_from = datetime.strptime(att['name'],'%Y-%m-%d %H:%M:%S')
                            hour_to = datetime.strptime(att['name'],'%Y-%m-%d %H:%M:%S')
                            ...
                            if  hour_from.hour < 6:
                                if hour_to.hour < 6:
                                    ...
                                    dur2 = 0.00
                                    nh = att['name'][:10] + ' 06:00:00'
                                    nh1 = datetime.strptime(nh,'%Y-%m-%d %H:%M:%S')
                                    diff = nh1 - hour_from
                                    minutes = (diff.seconds + diff.microseconds / 1000000.0)/60
                                    hours = round(minutes/60, 2)
                                    hour1 += hours
                    ...
                    if wh>0:
                        wh = hour2str(wh)
                        today_xml = '<day num="%s"><wh>%s</wh><ng>%s</ng></day>' % ((today - month).days+1, (wh), (hour1))
                    elif wh==0.0 and leave != '':
                        today_xml = '<day num="%s"><wh>%s</wh><ng>%s</ng></day>' % ((today - month).days+1, (leave), (''))
                    else:
                        today_xml = '<day num="%s"><wh>%s</wh><ng>%s</ng></day>' % ((today - month).days+1, (''), (''))
                    dy=(today - month).days+1
                    days_xml.append(today_xml)
                    today, tomor = tomor, tomor + one_day
                total = hour2str(total)
                hn = hour2str(hn)                    
                today_xml = '<day num="%s"><wh>%s</wh><ng>%s</ng></day>' % ((today - month).days+1, (total), (hn))
                dy=(today - month).days+1
                days_xml.append(today_xml)    
        ...
        while day_diff1>0:
            if month+i<=12:
                ...
                if day_diff1>=30:
                    ...
                    width_dict[j]=x
                day_diff1=day_diff1-x
        date_xml += ['<dayy number="%d" name="%s" cell="%d"/>' % (eom.day+1, 'Total',eom.day+1)]
        date_xml.append('</days>')
...
report_custom('report.hr.employee.attendance.bymonth', 'hr.employee', '', 'addons/l10n_ro_hr_payroll/report/att_bymonth.xsl')
