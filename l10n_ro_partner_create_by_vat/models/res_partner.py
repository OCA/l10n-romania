            ...
            if not self.env.context.get("skip_ro_vat_change"):
                ...
                if vat_country == "RO":
                    ...
                    if not anaf_error:
                        ...
                        # Uupdate l10n_ro_vat_subjected separately to make sure that when
                        # _fix_vat_number is called, value on l10n_ro_vat_subjected is already set
                        self.with_context(skip_ro_vat_change=True).update({"l10n_ro_vat_subjected": res.get("l10n_ro_vat_subjected", False)})
