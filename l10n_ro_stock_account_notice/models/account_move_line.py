...
class AccountMoveLine(models.Model):
    ...
    def _compute_account_id(self):
        l10n_ro_lines_for_notice = self.filtered(lambda x: x.product_id.type == "product" and x.is_l10n_ro_record)
        if l10n_ro_lines_for_notice:
            ...
            if move_id.is_purchase_document():
                ...
                if purchase and any(
                    [
                            p.purchase_method == "receive"
                            for p in self.mapped("product_id")
                        ]
                    ):
