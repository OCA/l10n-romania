...
class MT940Parser(models.AbstractModel):
    ...
    def get_header_regex(self):
        if self.get_mt940_type() == "mt940_ro_exim":
            # Eximbank files are wrapped in the standard SWIFT envelope
            # ({1:...}{2:...}{4:...}), so the header is the {1: block and not
            # the :20: tag. Since 18.0 the base parser matches the header
            # regex at the start of the data (re.match), so returning ":20:"
            # would make the file unrecognized.
            return "^{1:"
