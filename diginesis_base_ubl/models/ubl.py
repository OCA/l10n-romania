...
class BaseUbl(models.AbstractModel):
	...
	def _ubl_add_address(self, partner, node_name, parent_node, ns, version="2.1"):
		...
		if partner.state_id:
			...
			state.text = "{0}-{1}".format(partner.country_id and partner.country_id.code or '', partner.state_id.code)			
	...
	def _ubl_add_contact(
		...
		if partner.parent_id:
			...
			contact_name.text = self._prepare_partner_name(partner.name or partner.parent_id.name)
	...
	def _ubl_add_party_tax_scheme(
		...
		#Add node only for VAT partners
	...
	def _ubl_add_party_legal_entity(
		self, commercial_partner, parent_node, ns, party_type, version="2.1"
	):
		...
		registration_name.text = self._prepare_partner_name(commercial_partner.name)
		#Added according to ANAF
		company_id = etree.SubElement(party_legal_entity, ns["cbc"] + "CompanyID")
		company_id.text = commercial_partner.vat		
		
		if party_type == 'supplier' and self.comment:
			company_legal_form_id = etree.SubElement(party_legal_entity, ns["cbc"] + "CompanyLegalForm")
			company_legal_form_id.text = self._prepare_invoice_comment(self.comment)

	@api.model
	def _ubl_add_party(
		self, partner, company, node_name, parent_node, ns, party_type, version="2.1"
	):
		""" we remove WebsiteURI for now as it does not seem to be allowed by  DUK """
		...
		#Comment does not validate with ANAF
		#party_name = etree.SubElement(party, ns["cac"] + "PartyName")
		#name = etree.SubElement(party_name, ns["cbc"] + "Name")
		#name.text = self._prepare_partner_name(commercial_partner.name)
		#if partner.lang:
		...
		#Add party legal entity for all - not just companies
		#if company:
		self._ubl_add_party_legal_entity(
			commercial_partner, party, ns, party_type, version="2.1"
		)
		#Comment does not validate with ANAF
...
# 		partner_ref = self._ubl_get_customer_assigned_id(partner) #<<< DUK does not allow
# 		if partner_ref:
...
# 			customer_ref.text = partner_ref
		self._ubl_add_party(
			partner, company, "Party", customer_party_root, ns, party_type='customer', version=version
		)
		# TODO: rewrite support for AccountingContact + add DeliveryContact
		# Additional optional args
#  		if partner and not company and partner.parent_id: # <<< DUK does not allow
...
# 			supplier_ref.text = partner_ref
		self._ubl_add_party(
			partner, company, "Party", supplier_party_root, ns, party_type='supplier', version=version
		)
	...
	def _ubl_add_delivery(self, delivery_partner, parent_node, ns, version="2.1"):
		...
		#Comment for ANAF validation
		#self._ubl_add_party(
		#	delivery_partner, False, "DeliveryParty", delivery, ns, party_type='delivery', version=version
		#)
	...
	def _ubl_add_item(
		...
		item_description = self._prepare_item_description(name)
		if item_description:
		...
		name_node.text = self._prepare_item_name(name.split("\n")[0])
...
##commented on 04.07 for some reason
# 		if seller_code:
...
# 				std_identification_id.text = product.barcode
	...
	def _ubl_add_tax_category(
		...
		# Remove schemeID="UN/ECE5305"
		tax_category_id = etree.SubElement(
			#tax_category, ns["cbc"] + "ID", schemeID="UN/ECE5305")
			tax_category, ns["cbc"] + "ID")
		tax_category_id.text = tax.unece_categ_code
		#Add Percentage subelement for tax category
		tax_category_percentage = etree.SubElement(
				tax_category, ns["cbc"] + "Percent"
			)
		tax_category_percentage.text = str(float_round(tax.amount,precision_digits=2))
	...
	def _ubl_add_tax_scheme(self, tax_scheme_dict, parent_node, ns, version="2.1"):
		#Create Taxscheme only for VAT partners
		if tax_scheme_dict.get("id"):
			tax_scheme = etree.SubElement(parent_node, ns["cac"] + "TaxScheme")
			#Remove schemeID="UN/ECE5153" - not accepted by ANAF
			tax_scheme_id = etree.SubElement(
				#tax_scheme, ns["cbc"] + "ID", schemeID="UN/ECE5153")
	...
	@api.model
	def _prepare_partner_name(self, name):
		return (name or '')[:200]
	
	@api.model
	def _prepare_street(self, street):
		return (street or '')[:150]
	
	@api.model
	def _prepare_city(self, city):
		return (city or '')[:50]
	
	@api.model
	def _prepare_item_name(self, name):
		return (name or '')[:100]
	
	@api.model
	def _prepare_item_description(self, name):
		return (name or '')[100:300]
	
	@api.model
	def _prepare_invoice_comment(self, comment):
		return (comment or '')[:1000]
